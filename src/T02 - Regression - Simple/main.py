...
y = 1 / (np.power(X, 2) + 1) * 10 + np.random.random((nPoint,))
...
model = Sequential([
    ...
    normalizer,
    Dense(units=3, activation='sigmoid', kernel_initializer='normal', name='hidden1'),
...
earlyStoppingCallback = EarlyStopping(monitor='loss', patience=10, min_delta=0)
...
# Train the model
model.fit(xs, ys, epochs=1000, verbose=1, callbacks=[earlyStoppingCallback ])
